 ...
 *
 * Para cron via web (DonDominio), usar: public/cron/rfen_import.php?token=...
 */
...
// ── Consultar usuarios activos con RFEN vinculado ────────────────────────────

$sql = "SELECT id, nombre, rfen_id FROM users WHERE estado='activo' AND nadador_activo=1 AND rfen_id IS NOT NULL AND rfen_id != ''";
$params = [];
...
foreach ($users as $user) {
    ...
    if (!empty($result['error'])) {
        echo "[{$ts}] ERROR {$nombre} (rfen:{$rfen_id}): " . $result['error'] . PHP_EOL;
        $has_errors = true;
    } else {
        ...
        echo "[{$ts}] {$nombre} (rfen:{$rfen_id}): {$procesadas} procesadas, {$insertadas} insertadas, {$actualizadas} actualizadas, {$sin_cambios} sin cambios" . PHP_EOL;
    }
