...
$nadadores = []; // uid => ['nombre','sexo','liga','nadador_activo','pruebas'=>[prueba=>pts]]
...
foreach ($nadadores as $nad) {
    arsort($nad['pruebas']); // pruebas ordenadas por puntos desc
    ...
    $nad['top']    = array_slice($nad['pruebas'], 0, $filterN, true); // [prueba=>pts] top N
    $filas[] = $nad;
}

// Orden: por suma N desc, desempate suma8 y nombre
usort($filas, static function ($a, $b) {
    return [$b['suma_n'], $b['suma8'], $a['nombre']] <=> [$a['suma_n'], $a['suma8'], $b['nombre']];
});

render_header('Puntos AQUA', 'socio-ranking');
?>
...
<div class="container page-content">
  ...
  <div class="ranking-tabs">
    ...
    <a href="/socio/puntos-aqua" class="tab--active">Puntos AQUA</a>
  </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <?php else: foreach ($filas as $i => $f): ?>
            <tr <?= $f['uid'] == $user['id'] ? 'style="background:#eef2ff;"' : '' ?>>
              <td>
              ...
              <td>
                ...
                <?= $f['uid'] == $user['id'] ? '<span class="badge badge-blue" style="margin-left:6px;">Tú</span>' : '' ?>
              </td>
